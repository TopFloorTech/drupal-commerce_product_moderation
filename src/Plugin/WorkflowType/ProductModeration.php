 ...
 * )
 */
class ProductModeration extends WorkflowTypeBase implements ContainerFactoryPluginInterface {
  ...
  public function initializeWorkflow(WorkflowInterface $workflow) {
    $workflow
      ...
      ->addTransition('save_as_draft', $this->t('Save as Draft'), ['draft', 'published'], 'draft')
  ...
  public function checkWorkflowAccess(WorkflowInterface $entity, $operation, AccountInterface $account) {
    if ($operation === 'view') {
      return AccessResult::allowedIfHasPermission($account, 'view commerce product moderation');
    }
  ...
  public function decorateState(StateInterface $state) {
    if (isset($this->configuration['states'][$state->id()])) {
      $state = new ContentModerationState($state, $this->configuration['states'][$state->id()]['published'], TRUE);
    }
    else {
  ...
  public function buildStateConfigurationForm(FormStateInterface $form_state, WorkflowInterface $workflow, StateInterface $state = NULL) {
    ...
    $form['published'] = [
      ...
      '#description' => $this->t('When a product reaches this state it should be published.'),

 ...
 */
class ModerationOptOutPublishProduct extends PublishProduct implements ContainerFactoryPluginInterface {
  ...
  public function access($entity, AccountInterface $account = NULL, $return_as_object = FALSE) {
    ...
    if ($entity && $this->moderationInfo->isModeratedEntity($entity)) {
      drupal_set_message($this->t("@bundle @label were skipped as they are under moderation and may not be directly published.", ['@bundle' => $entity->bundle(), '@label' => $entity->getEntityType()->getPluralLabel()]), 'warning');
